"""Generate a GitHub PR template for agentic changes.

Lives at `.github/PULL_REQUEST_TEMPLATE/agentic-change.md` so GitHub will offer it as a named
template (selectable via `?template=agentic-change.md` on the new-PR URL). Existing PR templates
are not overwritten — re-running `pr-template init` reports the file as skipped.
"""
...
from .file_ops import collect_text


PR_TEMPLATE_RELATIVE = Path(".github") / "PULL_REQUEST_TEMPLATE" / "agentic-change.md"


@dataclass(frozen=True)
class PrTemplateResult:
...
def write_pr_template(target: Path) -> PrTemplateResult:
    """Write `.github/PULL_REQUEST_TEMPLATE/agentic-change.md` if it doesn't already exist."""

    root = target.expanduser().resolve()
    files: list[Path] = []
    skipped: list[Path] = []
    collect_text(files, skipped, root / PR_TEMPLATE_RELATIVE, _agentic_pr_template())
    return PrTemplateResult(files=files, skipped=skipped)


def _agentic_pr_template() -> str:
    return """<!--
